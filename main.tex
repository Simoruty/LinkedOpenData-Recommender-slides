		...
		\begin{column}{0.7\textwidth}
			\begin{tabular}{l l}
				...
				\begin{tabular}{l r}
					...
					\small Bastardi senza gloria & \small -1\\
				\end{tabular}	\\
			\end{tabular}
		\end{column}
	\end{columns}
...
\begin{frame}
	...
	I \textit{k} film raccomandati saranno:
	
	
	Titanic = (2 * 0.8 * 0.8) + ( 1 * 0.5 * 0.5) = 1.28 + 0.25 = \textbf{1.53} \footnote{(votAvatar * director * director)+ (votDjango * actor * actor)}
		
	Inglourious Bastards = (1 * 0.5 * 0.5) + (1 * 0.8 * 0.8) = 0.25 + 0.64 = \textbf{0.89} \footnote{(votDjango * actor * actor)+ (votDjango * director * director)}
\end{frame}
