		...
		\begin{column}{0.7\textwidth}
			\begin{tabular}{l l}
				...
				\begin{tabular}{l r}
					...
					\small Titanic & \small 1 \\
					\small Django Unchained & \small 1 \\
					\small Inglourious Basterds & \small -1\\
				\end{tabular}	\\
			\end{tabular}
		\end{column}
	\end{columns}
...
\begin{frame}
\frametitle{Grafo dei Film}
Riprendendo per comodità il grafico d'esempio
\\~\\
\begin{tikzpicture}
[lineDecoratein/.style={<-,thick}, %
lineDecorateout/.style={->,thick}, %
...
\begin{frame}
	...
	I \textit{k}$(=2)$ film raccomandati saranno, nell'ordine:
	
	\begin{equation*}
	Django Unchained \footnote{w(IB)*w(dir)*w(dir) + w(IB)*w(act)*w(act) + w(Tit)*w(act)*w(act)} = (2 * 0.8 * 0.8) + ( 2 * 0.5 * 0.5) + ( 1 * 0.5 * 0.5) = 
	\end{equation*}
	\begin{equation*}
	\qquad = \quad 1.28 + 0.5 + 0.25 \quad = \quad \textbf{2.03} 
	\end{equation*}
		
	\begin{equation*}
	Avatar \footnote{w(Titanic)*w(director)*w(director)} \quad = \quad (1 * 0.8 * 0.8) \quad = \quad \textbf{0.64}
	\end{equation*}
\end{frame}
